...
$('#transDate').datepicker({
  ...
  $('#button-add-sparepart').on('click', function(el) {
    ...
    var newAttribute = `<tr id="sparepart-row-${rowCounter}">
                          ...
                          <td>
                            <input class="form-control" nosave="true" id="sparepart-qty-out-${rowCounter}" type="text" placeholder="Enter QTY" name="sparepart[${rowCounter}][qty_out]" autocomplete="false">
                          </td>
                        </tr>`
  ...
  function callbackSaveRelated(returnId){
    ...
    for ( index = 0; index < sparepartLength; index++ ) {
      if ( index == 0 ) {
        ...
        dtAccess.addItem("trans_source",returnId,"numeric","0","");
        dtAccess.addItem("trans_id","DISTRIBUTION","string","0","");
        dtAccess.addItem("part_id",$(`#sparepart-id-${index}`).val(),"numeric","0","");
        dtAccess.addItem("qty_in","0","numeric","0","");
        dtAccess.addItem("qty_out",$(`#sparepart-qty-out-${index}`).val(),"numeric","0","");
      }else{
        if( $(`#sparepart-autonumber-${index}`).val() == "" ){
        ...
        dtAccess.addItemRelated(index-1,"trans_source",returnId,"numeric","0","");
        dtAccess.addItemRelated(index-1,"trans_id","DISTRIBUTION","string","0","");
        dtAccess.addItemRelated(index-1,"part_id",$(`#sparepart-id-${index}`).val(),"numeric","0","");
        dtAccess.addItemRelated(index-1,"qty_in","0","numeric","0","");
        dtAccess.addItemRelated(index-1,"qty_out",$(`#sparepart-qty-out-${index}`).val(),"string","0","");
      }
    }
